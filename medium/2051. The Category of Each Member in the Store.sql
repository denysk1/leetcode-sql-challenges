...
select
    ...
    m.name,
    case 
        when count(p.charged_amount) / count(v.visit_date) * 100 >= 80 
            then "Diamond"
        when count(p.charged_amount) / count(v.visit_date) * 100 >= 50
            and count(p.charged_amount) / count(v.visit_date) * 100 < 80
            then "Gold"
        when count(p.charged_amount) / count(v.visit_date) * 100 < 50
            then "Silver"
        else "Bronze"
