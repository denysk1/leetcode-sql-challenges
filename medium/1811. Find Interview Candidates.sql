...
select
    distinct u.name, u.mail
from (
    select 
        user_id, 
        contest_id,
        lag(contest_id) over (partition by user_id order by contest_id asc) as prev_contest_id,
        lag(contest_id, 2) over (partition by user_id order by contest_id asc) as prev2_contest_id
    from cte 
    order by user_id asc, contest_id asc ) tmp
inner join Users u on u.user_id = tmp.user_id
where contest_id - prev_contest_id = 1 and contest_id - prev2_contest_id = 2

union 

select u.name, u.mail
from Contests c
inner join Users u on u.user_id = c.gold_medal
