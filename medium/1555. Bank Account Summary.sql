...
select 
    u.user_id,
    u.user_name,
    coalesce(u.credit + q.change, u.credit) as credit,
    case when coalesce(u.credit + q.change, u.credit) > 0 then "No" else "Yes" end as credit_limit_breached
from Users u
left join (
    select 
        tmp.user_id,
        sum(tmp.change) as "change"
    from (
        select
            paid_by as user_id,
