...
(
    select
        *,
        lag(month) over (partition by account_id) as prev_month
    from (
        select
            account_id,
            exceed,
        ...
        from (
            select
                t.account_id,
                case when sum(t.amount) over (partition by t.account_id, date_format(day,'%m-%Y')) > a.max_income then 1 else 0 end as exceed,
                date_format(day,'%m-%Y') as month
            from Transactions t
            inner join Accounts a on t.account_id = a.account_id
            where t.type = "Creditor"
        ) q
        group by q.account_id, q.month
        order by q.account_id asc, q.month asc
    ) tmp
    where tmp.exceed = 1
) tmp1
where period_diff(month, prev_month) = 1;
